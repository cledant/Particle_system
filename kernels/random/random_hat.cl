__kernel void random_hat(__global t_particle *particle, float min, float max, 
	uint2 randoms_x, uint2 randoms_y, uint2 randoms_z, float4 center)
{
	...
	particle[i].pos.x = (z * cos(angle)) * max + center.x;
	particle[i].pos.y = (z * sin(angle)) * max + center.y;
	particle[i].pos.z = radius + z + max  + center.z;
